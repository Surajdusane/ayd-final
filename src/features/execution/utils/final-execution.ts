...
export const OPERATION_EXECUTORS: Record<string, OperationExecutor> = {
  ...
  NUMBER_FORMAT_OPREATION: ({ node, getInput, runtime }) => {
    ...
    const decimalPlaces = parseInt(
      resolveInput({ node, getInput, runtime }, "Decimal Places", "Decimal Places") ?? "0"
    );
    ...
    if (format === "ungrouped") {
      ...
      formattedValue = Intl.NumberFormat("en-US", { notation: "compact", maximumFractionDigits: decimalPlaces }).format(
        num
      );
...
export function generateFinalOutput(edges: Edge[], formData: { handleId: string; value: any }[], plan: executionPlan) {
  ...
  const getInputValue = (nodeId: string, targetHandle: string) => {
    ...
    if (value === undefined) {
      console.warn(`No value found in runtime for sourceHandle "${edge.sourceHandle}" ` + `(from node ${edge.source})`);
    }
    return value;
  };
  ...
  for (const input of documentNode.data.dynamicInputs) {
    ...
    if (edge) {
      ...
      if (value === undefined) {
        const constructedKey = `${edge.source}-${edge.sourceHandle}`;
        value = runtime.get(constructedKey);
        console.log(`Looking for "${input.name}": tried "${edge.sourceHandle}", then "${constructedKey}"`);
      }
